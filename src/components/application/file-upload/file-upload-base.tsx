    ...
    const isFileTypeAccepted = (file: File): boolean => {
        ...
        return acceptedTypes.some((acceptedType) => {
            ...
            if (acceptedType.startsWith(".")) {
                ...
                // Only check extension if file actually has one (more than one part after splitting)
...
export const FileListItemProgressBar = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        ...
        >
            <FileTypeIcon className="size-10 shrink-0 dark:hidden !text-brand-600" type={type ?? "empty"} theme="light" variant={fileIconVariant ?? "default"} />
            <FileTypeIcon className="size-10 shrink-0 not-dark:hidden !text-brand-600" type={type ?? "empty"} theme="dark" variant={fileIconVariant ?? "default"} />
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li layout="position" className={cx("relative flex gap-3 overflow-hidden rounded-xl bg-primary p-4", className)}>
            ...
            />
            <FileTypeIcon className="relative size-10 shrink-0 dark:hidden !text-brand-600" type={type ?? "empty"} theme="light" variant={fileIconVariant ?? "solid"} />
            <FileTypeIcon className="relative size-10 shrink-0 not-dark:hidden !text-brand-600" type={type ?? "empty"} theme="dark" variant={fileIconVariant ?? "solid"} />
