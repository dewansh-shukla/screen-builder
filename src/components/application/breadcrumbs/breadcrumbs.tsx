...
const styles = {
  ...
  'text-line':
    'pl-2 gap-1.5 md:gap-2 py-2 after:pointer-events-none after:absolute after:inset-0 after:border-b after:border-t after:border-secondary',
...
const Breadcrumbs = ({
  ...
  const visibleItems = (() => {
    ...
    const lastItems = childrenArray.slice(
      -Math.floor((maxVisibleItems - 1) / 2),
    );
    const ellipsisItem = (
      <BreadcrumbItem
        isEllipsis
  ...
  return (
    <nav aria-label="Breadcrumbs" className={cx('min-w-0', className)}>
      <BreadcrumbsContext.Provider value={{ divider, type }}>
        <AriaBreadcrumbs className={cx('relative flex flex-wrap', styles[type])}>
          {visibleItems}
        </AriaBreadcrumbs>
