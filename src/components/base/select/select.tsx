...
// Don't import ComboBox here to avoid circular dependency initialization error
// We'll use a lazy getter instead
...
// Define the extended type without referencing ComboBox in typeof to avoid initialization error
// Using unknown prevents TypeScript from trying to evaluate ComboBox at type definition time
type SelectWithSubcomponents = typeof Select & {
    ComboBox: unknown;
    Item: typeof SelectItem;
};

const _Select = Select as SelectWithSubcomponents;
_Select.Item = SelectItem;

// Use a lazy getter for ComboBox to break the circular dependency
// This ensures ComboBox is only loaded when accessed, after both modules are fully initialized
let _comboBoxCache: unknown = null;
Object.defineProperty(_Select, "ComboBox", {
    get() {
        if (!_comboBoxCache) {
            // Import ComboBox lazily - this breaks the circular dependency
            // because by the time this getter is called, both modules are fully loaded
            // eslint-disable-next-line @typescript-eslint/no-require-imports
            _comboBoxCache = require("./combobox").ComboBox;
        }
        return _comboBoxCache;
    },
    configurable: true,
    enumerable: true,
});
