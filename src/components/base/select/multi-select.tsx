    ...
    <div className="relative flex w-full flex-1 flex-row flex-wrap items-center justify-start gap-1.5">
      {!isSelectionEmpty &&
        comboBoxContext?.selectedItems?.items?.map(value => (
          <span
            key={value.id}
            className="bg-primary ring-primary flex items-center rounded-md py-0.5 pr-1 pl-1.25 ring-1 ring-inset"
          >
            <Avatar size="xxs" alt={value?.label} src={value?.avatarUrl} />

            <p className="text-secondary ml-1.25 truncate text-sm font-medium whitespace-nowrap select-none">
              {value?.label}
            </p>
  ...
  return (
    ...
    >
      {({ isDisabled }) => (
        <>
          {Icon && (
            <Icon className="text-fg-quaternary pointer-events-none size-5" />
          )}
