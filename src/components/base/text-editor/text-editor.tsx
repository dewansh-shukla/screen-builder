...
const TextEditorRoot = ({
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                ...
                `,
                class: cx(
                    "w-full resize scroll-py-3 overflow-auto rounded-lg bg-primary px-4 py-4 text-md leading-[1.5] text-primary caret-fg-brand-primary shadow-xs ring-1 ring-border-primary transition duration-100 ease-linear ring-inset selection:bg-fg-brand-primary/10 placeholder:text-fg-quaternary autofill:rounded-lg autofill:text-primary focus:ring-2 focus:ring-brand focus:outline-hidden break-words whitespace-pre-wrap",
    ...
    // Link functionality removed - Link extension not included in StarterKit
    // If link functionality is needed, import and add Link extension separately
    ...
    return (
        <EditorContext.Provider value={{ editor, isDisabled, limit, isInvalid, editorId }}>
            <div className={cx("flex w-full flex-col gap-2", className)}>{children}</div>
...
type TextEditorLabelProps = ComponentProps<typeof Label>;
...
type TextEditorHintTextProps = HTMLAttributes<HTMLElement>;
...
const TextEditorHintText = ({ children, ...props }: TextEditorHintTextProps) => {
    ...
    return (
        <HintText {...props} id={`${editorId}-hint`} isInvalid={isInvalid || exceedsLimit} className={cx("text-sm text-quaternary font-normal tracking-tight", limit && "tabular-nums", props.className)}>
