...
const Root = ({
  ...
  return (
    <PinInputContext.Provider value={{ size, disabled, id, hasError }}>
      <div
        role="group"
...
const Slot = ({
  ...
  const { size, disabled, hasError } = usePinInputContext();
  ...
  return (
    <div
      ...
      className={cx(
        'bg-primary text-placeholder_subtle ring-primary relative flex items-center justify-center rounded-xl text-center shadow-xs ring-1 transition-[box-shadow,background-color] duration-100 ease-linear ring-inset',
        sizes[size],
        isFocused &&
          slot?.isActive &&
          'ring-brand outline-brand ring-2 outline-2 outline-offset-2',
        slot?.char && 'text-brand-tertiary_alt ring-brand ring-2',
        disabled && 'bg-disabled_subtle text-fg-disabled_subtle ring-disabled',
        hasError && 'ring-red-500 border-red-500 ring-2',
        className,
      )}
    >
      {slot?.char ? (
...
const FakeCaret = ({ size = 'md' }: { size?: 'sm' | 'md' | 'lg' }) => {
  return (
    <div
      className={cx(
        'animate-caret-blink bg-fg-brand-primary pointer-events-none h-[1em] w-0.5',
        size === 'lg'
...
const Separator = (props: ComponentPropsWithRef<'p'>) => {
  return (
    <div
      ...
      {...props}
      className={cx(
        'text-display-xl text-placeholder_subtle text-center font-medium',
        props.className,
      )}
    >
      -
...
const Label = ({ className, ...props }: ComponentPropsWithRef<'label'>) => {
  ...
  return (
    <label
      ...
      className={cx('text-secondary text-sm font-medium', className)}
    />
  );
};
...
const Description = ({ className, ...props }: ComponentPropsWithRef<'p'>) => {
  ...
  return (
    <p
      ...
      role="description"
      className={cx('text-tertiary text-sm', className)}
    />
  );
};
